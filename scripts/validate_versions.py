#!/usr/bin/env python3
"""Validate that all pyproject.toml files declare the same version,
and that this version matches the given tag.

Usage:
    python scripts/validate_versions.py v0.4.0
    python scripts/validate_versions.py 0.4.0   # leading 'v' optional
...
PYPROJECTS = [
    "pyproject.toml",
    "packages/ernie-image-core-mlx/pyproject.toml",
]
...
def main(argv: list[str]) -> int:
    ...
    if mismatched:
        ...
        for rel, v in versions.items():
            marker = "FAIL" if rel in mismatched else "OK"
            print(f"  [{marker}] {rel}: {v}", file=sys.stderr)
        return 1

    print(f"all {len(PYPROJECTS)} pyproject.toml files agree: version = {expected}")
    return 0
